...
import { TabsetLayoutComponent } from '../components/tabset-layout/tabset-layout.component';
import { createRandomString, OptionalAttribute} from 'ng-bootstrap-addons/utils';
...
const TabAutoOpenGuard: (tabs: TabRoute[]) => CanActivateFn = (tabs: TabRoute[]) => {
  return () => {
    const tabManager = inject(TabsetManagerService);
...
export class TabRoute implements Route {
  ...
  constructor(route: Omit<TabRoute, 'tab'> & { tab: OptionalAttribute<Omit<TabData, 'component'>, 'id'>; }) {
    const dataKey = 'tab';
    Object.assign(this, route);
    this.component = route.component;
    this.tab = { ...route.tab, component: route.component, id: route.tab.id ?? `tab-${createRandomString(12)}` };
...
const openTab = (tabConfig: TabData, tabManager: TabsetManagerService) => {
