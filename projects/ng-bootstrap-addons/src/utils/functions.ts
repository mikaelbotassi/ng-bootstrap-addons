...
export const convertBlobToFile = (propertie:{nome:string, item:any}):File => {
...
export const assignMatchingProperties = <T>(target: T, source: Partial<T>): void =>{}

export const fromObjectToParams = (data:Object):HttpParams => {
    let params = new HttpParams();
    
    for(let key in data)
        if(isset(data[key as keyof typeof data])) params = params.set(key, String(data[key as keyof typeof data]));
...
export const fromFormObjectToParams = (data:Object):HttpParams => {
    ...
    for(let key in data){
        ...
        if(control instanceof FormControl && isset(control.value)) params = params.set(key, String(control.value));
    }
