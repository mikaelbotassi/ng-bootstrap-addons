...
export class DatePickerComponent extends ControlValueAccessorDirective<Date|undefined>{

  readonly collisionId = `date-picker-${createRandomString(20)}`;
  ...
  baseConfigs = computed<Partial<BsDaterangepickerConfig>>(() => {
    ...
    return {
      ...
      ranges: withTime ? [
        { label: 'Hoje', value: DateUtils.toDate(DateUtils.getFormattedCurrentDate('YYYY-MM-DD 00:00:00')) },
      ] : [
        { label: 'Hoje', value: DateUtils.toDate(DateUtils.getFormattedCurrentDate('YYYY-MM-DD'))},
      ],
    };
  });
